# linux-toturial

<div dir="rtl">

# دستورات `pwd` و `cd`

## دایرکتوری چیست؟

دایرکتوری (Directory) یک پوشه است که حاوی لیستی از فایل‌ها و/یا دایرکتوری‌های دیگر است. این فایل‌ها ممکن است عکس، فیلم، متن و یا حتی دایرکتوری دیگری باشند. در واقع دایرکتوری یک ساختار برای سازمان‌دهی فایل‌ها در کامپیوتر است.

</div>

```
Project
├── GitLab.jpg
├── Intro.txt
├── Linux
│   ├── Manjaro
│   └── Ubuntu.png
└── video.mp4

3 directories, 4 files
```
<div dir="rtl">

**توضیحات :**

* `.` شاخص دایرکتوری فعلی (Current Directory)
* `..` شاخص دایرکتوری سطح بالاتر (Parent Directory)
* Subdirectoryها زیرشاخه‌های دایرکتوری محسوب می‌شوند.

## دستور `pwd`

این دستور (Print Working Directory) مسیر کامل دایرکتوری فعلی را نمایش می‌دهد:

</div>

```terminal
user@Machine:~$ pwd
/home/user
user@Machine:~$
```
<div dir="rtl">

## دستور `cd`

این دستور (Change Directory) برای جابجایی بین دایرکتوری‌های مختلف استفاده می‌شود:

### ۱. وارد کردن آدرس مقصد بصورت کامل

آدرس کامل (Absolute Path) همیشه از ریشه (`/`) شروع می‌شود:

</div>

```terminal
user@Machine:~$ cd /home/USERNAME/Desktop
```

<div dir="rtl">

### ۲. آدرس نسبی (Relative Path)

مسیر نسبی نسبت به دایرکتوری فعلی تعیین می‌شود. برای مثال اگر در `/home/user` باشید:

</div>

```terminal
# وارد پوشه Desktop درون دایرکتوری فعلی
user@Machine:~$ cd Desktop
user@Machine:~/Desktop$
```

<div dir="rtl">

### ۳. بازگشت به دایرکتوری سطح بالاتر

با استفاده از `..` می‌توان به دایرکتوری مادر (Parent) بازگشت:

</div>

```terminal
user@Machine:~/Desktop$ cd ..
user@Machine:~$
```
<div dir="rtl">

### ۴. رفتن به دایرکتوری خانه کاربر

استفاده از `cd` بدون آرگومان یا `cd ~` کاربر را به پوشه خانگی‌اش برمی‌گرداند:

</div>

```terminal
user@Machine:~/Downloads$ cd
user@Machine:~$
```
<div dir="rtl">

### ۵. استفاده از `-` برای بازگشت به دایرکتوری قبلی

</div>

```terminal
user@Machine:~/Projects$ cd ~/Downloads
user@Machine:~/Downloads$ cd -
/home/user/Projects
user@Machine:~/Projects$
```


<div dir="rtl">

# دستور `ls`

این دستور برای مشاهدهٔ محتویات فایل‌ها و دایرکتوری‌ها در لینوکس استفاده می‌شود. در صورت استفاده از این دستور در بخش فرمان (Terminal)، نام فایل‌ها و دایرکتوری‌های موجود در دایرکتوری فعلی نمایش پیدا می‌کنند.

## مثال

</div>

```terminal
user@machine:~/ls-commands$ ls
linux.jpg my_folder photos tasks.txt
```

<div dir="rtl">

## لیست فایل‌ها بر اساس زمان (time) یا حجم (size)

برای مرتب‌سازی فایل‌ها بر اساس تاریخ (time) یا حجم (size) می‌توان از آپشن `-t` یا `-S` استفاده کرد.

### مرتب‌سازی بر اساس زمان (جدیدترین اول)

</div>

```terminal
user@machine:~/ls-commands$ ls -t
linux.jpg  photos  my_folder  tasks.txt
```

<div dir="rtl">

### مرتب‌سازی بر اساس حجم (بزرگ‌ترین اول)

</div>

```terminal
user@machine:~/ls-commands$ ls -S
linux.jpg  photos  my_folder  tasks.txt
```

<div dir="rtl">

## نمایش جزئیات (Long Listing)

آپشن `-l`:

</div>

```terminal
user@machine:~/ls-commands$ ls -l
total 200
drwxrwxr-x 2 user user  4096 Aug 10 10:54 linux.jpg
drwxr-xr-x 4 user user  4096 Aug 10 11:03 my_folder
-rw-r--r-- 1 user user  4096 Aug 10 15:09 tasks.txt
```

<div dir="rtl">

**توضیحات خروجی:**

* ستون اول نوع فایل و مجوزها (Permission).
* ستون دوم تعداد لینک‌های سخت (Hard Links).
* ستون سوم و چهارم مالک (Owner) و گروه (Group).
* ستون پنجم حجم فایل به بایت.
* ستون ششم تاریخ و زمان آخرین تغییرات (Modification Time).
* ستون هفتم نام فایل یا دایرکتوری.

## نمایش فایل جدید در یک مسیر دلخواه

برای ایجاد فایل جدید و مشاهدهٔ محتویات آن:

</div>


```bash
# ساخت فایل جدید
> echo "Hello World" > /tmp/new_hello
# نمایش دستور ls برای مسیر جدید
user@machine:~/ls-commands$ ls -l /tmp/new_hello
-rw-r--r-- 1 user user   12 Aug 10 15:09 /tmp/new_hello
# نمایش محتوای فایل
user@machine:~/ls-commands$ cat /tmp/new_hello
Hello World
```

<div dir="rtl">

ستون سوم و چهارم مالک و گروه فایل هستند.

نکته: اگر حجم فایل در صفر کیلوبایت باشد، نشان‌دهندهٔ خالی بودن فایل است. همچنین زمان آخرین تغییر در دایرکتوری مربوطه نیاز به دقت دارد زیرا زمان آخرین تغییر در دایرکتوری به معنی اضافه/حذف فایل‌ها است.

## استفاده از آپشن‌ها به همراه نام دایرکتوری

</div>

```terminal
user@machine:~/ls-commands$ ls -a
.  ..  linux.jpg  my_folder  photos  tasks.txt
```

<div dir="rtl">

مشاهدهٔ همهٔ فایل‌ها شامل فایل‌های مخفی (`.` و `..`) و دیگر فایل‌ها.

</div>

```terminal
user@machine:~/ls-commands$ cd ..
user@machine:~/another-directory$ ls -l
# خروجی مشابه با قبل، فقط در دایرکتوری دیگر نمایش داده می‌شود.
```

<div dir="rtl">

## مشاهدهٔ فقط فایل‌ها یا دایرکتوری‌ها

* آپشن `-d`: فقط دایرکتوری‌ها را نمایش دهد.
* آپشن `-F`: در انتهای نام دایرکتوری‌ها `/` اضافه می‌کند.

</div>

```terminal
user@machine:~/ls-commands$ ls -F
linux.jpg  my_folder/  photos/  tasks.txt
```

<div dir="rtl">

## ترکیب چند آپشن در یک دستور

می‌توان چند آپشن را به صورت ترکیبی استفاده کرد. معمولاً ابتدا `-l` و پس از آن سایر آپشن‌ها می‌آیند. ترتیب آپشن‌ها تأثیری در خروجی ندارد.

</div>

```terminal
user@machine:~/ls-commands$ ls -ltr
total 200
drwxr-xr-x 4 user user  4096 Aug 10 11:03 my_folder
-rw-r--r-- 1 user user  4096 Aug 10 15:09 tasks.txt
-rw-r--r-- 1 user user  18041 Aug 10 10:54 linux.jpg
```

<div dir="rtl">

### توضیح آپشن‌ها در ترکیب:

* `-l`: نمایش جزئیات.
* `-t`: مرتب‌سازی بر اساس زمان آخرین تغییر.
* `-r`: معکوس کردن ترتیب (قدیمی‌ترین اول).
* `-h`: خواناتر کردن حجم فایل به کیلوبایت/مگابایت.

</div>



```terminal
user@machine:~/ls-commands$ ls -lh
total 200K
-rw-r--r-- 1 user user  4.0K Aug 10 15:09 tasks.txt
-rw-r--r-- 1 user user 12K   Aug 10 10:54 linux.jpg
drwxr-xr-x 4 user user  4.0K Aug 10 11:03 my_folder
```


<div dir="rtl">

> با ترکیب آپشن‌های `-l`, `-h`, `-t`, و `-r` می‌توانید خروجی مناسبی از دستور `ls` داشته باشید که به صورت خوانا و مرتب شده نمایش داده می‌شود.

</div>

<div dir="rtl">

# تکمیل کار با ترمینال

در این درس‌نامه می‌خواهیم با ترمینال مانند یک حرفه‌ای کار کنیم. برای اینکار ابتدا با چند میانبر مهم از ترمینال آشنا می‌شویم و سپس یاد می‌گیریم که چطور تاریخچه خود را در ترمینال مشاهده و در صورت نیاز پاک کنیم. توجه داشته باشید که اکثر میانبرها برای ترمینال اوبونتو نوشته شده است. اگر از ترمینال و یا سیستم‌عامل دیگری استفاده می‌کنید ممکن است میانبرها یا خروجی‌های شما تفاوت داشته باشد.

---

## باز کردن ترمینال جدید

برای باز کردن یک ترمینال جدید، می‌توانید از میانبر `Ctrl + Alt + T` استفاده کنید.

## باز کردن تب جدید در ترمینال

می‌توانید با زدن `Ctrl + Shift + T` و یا از طریق کلیک روی علامت `+` موجود در بالا سمت راست پنجره ترمینال یک تب جدید به ترمینال‌تان اضافه کنید.

## جابه‌جایی میان ترمینال‌ها

برای جابه‌جا شدن میان تب‌های مختلف ترمینال‌تان می‌توانید از میانبرهای `Ctrl + PgUp` و `Ctrl + PgDn` استفاده کنید.

## پاک‌کردن صفحه ترمینال

برای پاک‌کردن صفحه ترمینال‌تان می‌توانید از میانبر `Ctrl + L` استفاده کنید.

---

## دیدن تاریخچه دستورات

برای مشاهده همه دستوراتی که از آن‌ها استفاده و در ترمینال وارد کرده‌اید می‌توانید از دستور `history` استفاده کنید.

</div>

```bash
user@Machine:~$ history
 1  pwd
 2  cd /home/user/Desktop/
 3  cd ~
 4  cd Desktop/
 5  pwd --help
 6  help pwd
 7  ls
 8  ls -t
 9  ls -l
10  ls -a
11  ls -r
12  ls -l -t
13  ls -lt
14  ls -ltr
15  history
user@Machine:~$
```

<div dir="rtl">

---

## دیدن تعدادی از دستورات اخیر

اگر بخواهید چند دستور آخر ترمینال‌تان را مشاهده کنید، می‌توانید تعداد موردنظرتان را در جلوی دستور `history` بنویسید.

مثال:

</div> 

```bash
history 5
```

<div dir="rtl">

خروجی:

</div>

```bash
 13  ls -l -t
 14  ls -lt
 15  ls -ltr
 16  history
 17  history 5
```

<div dir="rtl">

---

## پاک‌کردن تاریخچه دستورات

برای اینکه تاریخچه دستوراتی مورد استفاده‌تان را خالی کنید، می‌توانید از دستور زیر استفاده کنید:

</div>

```bash
history -c
```

<div dir="rtl">

# دستورات mkdir و rmdir

در این درس‌نامه می‌خواهیم با نحوه ایجاد و حذف دایرکتوری از طریق ترمینال آشنا شویم.

---

## دستور `mkdir`

دستوری که از آن برای ساخت پوشه (Make Directory) با کمک این دستور می‌توانیم یک دایرکتوری بسازیم. برای مثال:

</div>

```bash
mkdir Test
```

<div dir="rtl">

### ساخت چند پوشه با یک دستور

برای ساخت چند پوشه به صورت هم‌زمان می‌توانیم به صورت زیر عمل کنیم:

</div>

```bash
mkdir a b c
```

<div dir="rtl">

### ساخت پوشه همراه با flag

می‌توانیم از `-v` برای گرفتن خروجی نیز استفاده کنیم:

</div>

```bash
mkdir -v a b c
```

<div dir="rtl">

### نحوه ایجاد دایرکتوری با وجود داشتن زیر دایرکتوری

برای مثال اگر بخواهیم دایرکتوری به نام `inside` و سپس دایرکتوری دیگری با نام `inside-a` ایجاد کنیم:

</div> 

```bash
mkdir -p inside-a/inside-inside-a
```

<div dir="rtl">

---

## دستور `rmdir`

دستوری که از آن برای حذف پوشه (Remove Directory) با کمک این دستور می‌توانیم یک دایرکتوری را حذف کنیم.

</div> 

```bash
mkdir Test
rmdir Test
```

<div dir="rtl">

در صورتی که دایرکتوری مورد نظر خالی نباشد (یعنی فایل یا پوشه دیگری درون آن باشد) با پیغام خطای زیر مواجه می‌شوید:

</div>

```bash
rmdir: failed to remove 'Test': Directory not empty
```

<div dir="rtl">

---

## ساخت تو در تو و حذف آن

برای مثال:

</div> 

```bash
mkdir -p a/b/c/
```

<div dir="rtl">

برای حذف آن:

</div> 

```bash
rm -r a/
```

<div dir="rtl">

---

## تمرین پایانی

ساختار فایل زیر را ایجاد کنید:

</div>

```
app
├── node_modules
├── public
│   ├── images
│   └── 2020
```

<div dir="rtl">

سپس کل این ساختار را پاک کنید.

### پاسخ:

</div>

```bash
mkdir -p app/node_modules app/src app/public/{images/2020}
rm -rf app/
```

<div dir="rtl">

# دستورات `touch` و `cat`

در این درس می‌خواهیم با دستورات `touch` که برای ایجاد یک فایل جدید و `cat` که برای مشاهدهٔ محتوای فایل‌هاست، آشنا شویم.

---

## دستور `touch`

می‌توانیم با استفاده از دستور `touch` فایل جدیدی بسازیم. به‌عنوان مثال، با کمک دستور زیر، فایلی به‌نام `a.txt` را در دایرکتوری جاری ایجاد کنیم:

</div>

```
touch a.txt
```

<div dir="rtl">

می‌توانیم چندین فایل با یک دستور بسازیم:

</div>

```
touch a.txt b.txt c.txt
```

<div dir="rtl">

### اگر فایل وجود داشته باشد

اگر فایلی با نام مشخص‌شده از قبل وجود داشته باشد، دستور `touch` صرفاً تاریخ آخرین دسترسی یا ویرایش آن را به‌روزرسانی می‌کند.

---

## نحوهٔ ایجاد فایل با نامی خاص که فاصله دارد

برای فایل‌هایی که نامشان دارای فاصله است (مثلاً `example 1.txt`)، باید نام فایل را در دابل‌کوتیشن قرار دهیم:

</div>

```
touch "example 1.txt" "example 2.txt"
```

<div dir="rtl">

یا:

</div>

```
touch a\ example\ b\ example.txt
```

<div dir="rtl">

---

## دستور `cat`

با استفاده از دستور `cat` می‌توانیم محتوای فایل‌ها را مشاهده کنیم:

</div>

```
cat sample.txt
```

<div dir="rtl">

### نمونه‌ای از خروجی:

</div>

```
Hello
This is a test document for learning cat command
Goodbye!
```

<div dir="rtl">

---

### مشاهده با شماره خط

برای مشاهدهٔ محتوا با شماره خط از دستور زیر استفاده می‌کنیم:

</div>

```
cat -n sample.txt
```

<div dir="rtl">

---

## نحوهٔ نوشتن دستی فایل با استفاده از دستور `cat`

برای نوشتن محتوا به‌صورت دستی:

</div>

```
cat > show.txt
```

<div dir="rtl">

سپس محتوا را تایپ می‌کنیم و در پایان با `Ctrl + D` از حالت وارد کردن خارج می‌شویم.

---

### مشاهده محتوای فایل ساخته‌شده:


</div>

```
cat show.txt
```

<div dir="rtl">

خروجی نمونه:

</div>

```
this is a sample test to written files with terminal.
using cat command and redirection.
```

<div dir="rtl">

---

## تمرین نهایی

### ساختار فایل زیر را ایجاد کنید:

</div>

```
app
├── node_modules
├── public
│   └── images
│       └── 2022
├── src
│   └── index.js
└── package.json
```

<div dir="rtl">

### سپس کل این ساختار را پاک کنید:

</div>

```bash
mkdir -p app/node_modules app/public/images/2022
touch src/index.js package.json
rm -rf app/
```

<div dir="rtl">

---

# دستورات `cp`، `mv` و `rm`

در این درس‌نامه می‌خواهیم با نحوهٔ کپی، انتقال، حذف و تغییر نام فایل‌ها و دایرکتوری‌ها در ترمینال آشنا شویم.

---

## دستور `cp`

دستور `cp` (کپی) با گرفتن آدرس یک فایل و آدرس مقصد، فایل را در مقصد کپی می‌کند:

</div>

```bash
cp a.txt test/b.txt
```

<div dir="rtl">

اگر `a.txt` در دایرکتوری فعلی و `test/` دایرکتوری مقصد باشد، فایل `a.txt` در آنجا با نام `b.txt` کپی می‌شود.

کپی بین دو دایرکتوری:

</div>

```bash
cp test1/a.txt test2/b.txt
```

<div dir="rtl">

کپی دایرکتوری (با تمام محتویات):

</div>

```bash
cp -r dir1 test/dir2
```

<div dir="rtl">

---

## دستور `mv`

دستور `mv` (موو) برای انتقال یا تغییر نام فایل‌ها و دایرکتوری‌ها استفاده می‌شود. برخلاف `cp`، فایل یا پوشهٔ مبدا را حذف می‌کند.

انتقال فایل:

</div>

```bash
mv dir1/file dir2/
```

<div dir="rtl">

تغییر نام فایل:

</div>

```bash
mv file1 file2
```

<div dir="rtl">

انتقال دایرکتوری به مسیر جدید:

</div>

```bash
mv dir1/ ~/Desktop/
```

<div dir="rtl">

تغییر نام دایرکتوری:

</div>

```bash
mv dir1/ dir2/
```

<div dir="rtl">

---

## دستور `rm`

دستور `rm` (حذف) برای پاک کردن فایل‌ها و دایرکتوری‌ها استفاده می‌شود.

حذف فایل:

</div>

```bash
rm test/a.txt
```

<div dir="rtl">

حذف دایرکتوری به همراه محتوایش:

</div>

```bash
rm -r test/
```

<div dir="rtl">

---

## 🛠 تمرین کاربردی

### ساختار اولیه:

</div>

```
app
├── node_modules
├── public
│   └── images
│       └── 2022
│           └── index.html
├── src
│   └── index.js
└── package.json
```

<div dir="rtl">

### هدف: تبدیل ساختار بالا به:

</div>

```
app
├── public
│   └── images
│       └── 2022
├── index.html
├── src
│   └── images
│       └── 2022
│   └── index.js
└── package.json
```

<div dir="rtl">

### دستورها:

</div>

```bash
mv app/src/package.json app
cp -r app/public/images app/src/
rmdir app/node_modules
```

---
<div dir="rtl">

# دیدن زمان و تاریخ در ترمینال

در این درس‌نامه با دستورهای نمایش زمان و تاریخ در ترمینال آشنا می‌شویم.

---

## دستور `date`

برای نمایش تاریخ و زمان سیستم از دستور `date` استفاده می‌کنیم:

</div>

```bash
date
```
<div dir="rtl">

---

## نمایش فقط ساعت (با استفاده از فرمت‌دهی)

</div>

```bash
date +"%T"
```
<div dir="rtl">

یا:

</div>

```bash
date +"%H:%M:%S"
```

<div dir="rtl">

---

## نمایش فقط تاریخ

</div>

```bash
date +"%F"
```

<div dir="rtl">

یا:

</div>

```bash
date +"%Y-%m-%d"
```

<div dir="rtl">

---

## فرمت‌های متداول برای `date`

برخی از کاراکترهای فرمت‌دهی پرکاربرد برای دستور `date`:

| کاراکتر | معنی                   |
| ------- | ---------------------- |
| `%Y`    | سال چهار رقمی          |
| `%y`    | سال دو رقمی            |
| `%m`    | عدد ماه (01-12)        |
| `%B`    | نام کامل ماه           |
| `%b`    | نام مخفف ماه           |
| `%d`    | روز ماه (01-31)        |
| `%A`    | نام کامل روز هفته      |
| `%a`    | نام مخفف روز هفته      |
| `%H`    | ساعت (00-23)           |
| `%I`    | ساعت (01-12)           |
| `%M`    | دقیقه                  |
| `%S`    | ثانیه                  |
| `%p`    | قبل/بعد از ظهر (AM/PM) |

---

## دستور `time`

برای محاسبه زمان اجرای یک دستور از `time` استفاده می‌کنیم:

</div>

```bash
time ls
```
<div dir="rtl">

خروجی شامل:

* real: زمان واقعی سپری‌شده
* user: زمان مصرف‌شده توسط CPU در فضای کاربر
* sys: زمان مصرف‌شده توسط CPU در فضای kernel

---

## دستور `cal`

نمایش تقویم ماه جاری:

</div>

```bash
cal
```

<div dir="rtl">

---

## نمایش تقویم یک ماه خاص

</div>

```bash
cal 1 2025
```

<div dir="rtl">

---

## نمایش تقویم یک سال کامل

</div>

```bash
cal 2025
```

<div dir="rtl">

---

## نمایش تقویم سال جاری

</div>

```bash
cal -y
```

<div dir="rtl">

---

## نمایش تقویم با هفتهٔ اول شروع‌شده از دوشنبه

</div>

```bash
cal --monday
```

<div dir="rtl">

---

## نمایش تقویم با برجسته‌سازی تاریخ امروز

</div>

```bash
cal -h
```

---


## خلاصه دستورات

| دستور       | عملکرد                                        | مثال                                            |
|-------------|-----------------------------------------------|-------------------------------------------------|
| `pwd`       | نمایش مسیر کامل دایرکتوری جاری               | `$ pwd` → `/home/querа`                         |
| `cd /مسیر`  | جابجایی به دایرکتوری با مسیر مطلق            | `$ cd /home/querа/Desktop`                      |
| `cd نام_پوشه` | جابجایی به زیرپوشه (مسیریابی نسبی)           | `$ cd Desktop`                                  |
| `cd ..`     | رفتن به دایرکتوری والد (یک سطح بالاتر)       | `$ cd ..`                                       |
| `cd`        | بازگشت به پوشهٔ خانه (`~`)                    | `$ cd` → `/home/querа`                          |
| `cd -`      | بازگشت به دایرکتوری قبلی                     | `$ cd -`                                        |



| دستور     | عملکرد                                      | مثال واضح‌تر                                                           |
| --------- | ------------------------------------------- | ---------------------------------------------------------------------- |
| `ls`      | نمایش فایل‌ها و پوشه‌های دایرکتوری جاری     | `$ ls` → `linux.jpg  task.txt`                                         |
| `ls -t`   | مرتب‌سازی بر اساس زمان آخرین تغییر          | `$ ls -t` → `task.txt  linux.jpg`                                      |
| `ls -l`   | نمایش اطلاعات کامل فایل‌ها                  | `$ ls -l` → `-rw-r--r-- 1 user user  1054 Aug 10 10:52 task.txt`     |
| `ls -lt`  | اطلاعات کامل + مرتب‌سازی زمانی              | `$ ls -lt` → (task.txt اول، بعد linux.jpg با جزئیات)                   |
| `ls -a`   | نمایش فایل‌های مخفی                         | `$ ls -a` → `.  ..  .hidden_file  linux.jpg`                           |
| `ls -la`  | فایل‌های مخفی + اطلاعات کامل                | `$ ls -la` → `-rw-r--r-- 1 user user 1024 Aug 10 10:52 .hidden_file` |
| `ls -lS`  | مرتب‌سازی بر اساس اندازه فایل (بزرگ‌تر اول) | `$ ls -lS` → (بزرگ‌ترین فایل در خط اول)                                |
| `ls -ltr` | نمایش کامل + مرتب‌سازی زمانی معکوس          | `$ ls -ltr` → (قدیمی‌ترین فایل اول، جدیدترین آخر)                      |
| `ls -lh`  | نمایش اندازه‌ها به‌صورت خوانا               | `$ ls -lh` → `1.1K task.txt  512K linux.jpg`                           |
| `ls -lth` | ترکیب اطلاعات کامل، خوانایی اندازه، و زمان  | `$ ls -lth` → (همه‌ی اطلاعات با حجم قابل خواندن و زمان مرتب)           |



|  دستور       | عملکرد                                | توضیح / مثال                            |
| -------------------- | ------------------------------------- | --------------------------------------- |
| `Ctrl + Alt + T`     | باز کردن ترمینال جدید                 | از هر نقطه از محیط دسکتاپ اوبونتو       |
| `Ctrl + Shift + T`   | باز کردن تب جدید در ترمینال           | مشابه باز کردن تب در مرورگر             |
| `Ctrl + PgUp / PgDn` | جابه‌جایی بین تب‌های مختلف در ترمینال | حرکت بین تب‌های باز ترمینال             |
| `Ctrl + L`           | پاک‌کردن صفحه ترمینال (clear)         | مانند اجرای دستور `clear`               |
| `history`            | نمایش لیست کامل دستورات وارد شده      | `$ history` → مشاهدهٔ کل تاریخچه        |
| `history عدد`        | نمایش n دستور اخیر از تاریخچه         | `$ history 5` → نمایش ۵ دستور آخر       |
| `history -c`         | پاک‌کردن تاریخچهٔ دستورات             | بدون بازگشت، محتویات history پاک می‌شود |


| دستور                   | عملکرد                                       | مثال / توضیح                                             |
|-------------------------| -------------------------------------------- | -------------------------------------------------------- |
|  `mkdir نام‌پوشه`       | ساخت یک دایرکتوری جدید                       | `mkdir test` → ایجاد پوشه‌ی `test`                       |
| `mkdir a b c`           | ساخت چند دایرکتوری به‌صورت هم‌زمان           | ساخت سه پوشه‌ی مجزا: `a`, `b`, `c`                       |
| `mkdir -v ...`          | نمایش پیام پس از ساخت هر دایرکتوری           | `mkdir -v a b c` → پیام "created directory" برای هر پوشه |
| `mkdir -p مسیر/زیرمسیر` | ساخت دایرکتوری‌های تو در تو، در صورت نیاز    | `mkdir -p a/b/c` → ایجاد مسیر کامل با زیرپوشه‌ها         |
| `rmdir نام‌پوشه`        | حذف یک دایرکتوری (فقط اگر خالی باشد)         | `rmdir test` → فقط اگر پوشه خالی باشد                    |
| `rm -r پوشه`            | حذف پوشه حتی اگر شامل فایل یا پوشه دیگر باشد | `rm -r a/` → حذف بازگشتی همه‌چیز درون `a/`               |
| `rm -rf پوشه`           | حذف بدون درخواست تأیید + بازگشتی             | `rm -rf app/` → حذف سریع و قطعی کل ساختار دایرکتوری      |


| دستور             | عملکرد                    | مثال / توضیح                                  |
|-------------------| ------------------------- | --------------------------------------------- |
| `cp مبدا مقصد`    | کپی کردن فایل             | `cp a.txt test/b.txt` → کپی فایل با تغییر نام |
| `cp -r پوشه مقصد` | کپی پوشه با تمام محتویات  | `cp -r dir1 test/dir2`                        |
| `mv مبدا مقصد`    | انتقال فایل یا پوشه       | `mv dir1/file dir2/`                          |
| `mv فایل1 فایل2`  | تغییر نام فایل            | `mv file1 file2`                              |
| `mv dir1/ dir2/`  | تغییر نام یا جابجایی پوشه | نام جدید یا انتقال به مسیر جدید               |
| `rm فایل`         | حذف فایل                  | `rm a.txt`                                    |
| `rm -r پوشه`      | حذف دایرکتوری و محتوای آن | `rm -r test/`                                 |


## ⏱ خلاصه دستورات تاریخ و زمان در لینوکس

| دستور                         | کاربرد                                       |
| ----------------------------- | -------------------------------------------- |
| `date`                        | نمایش تاریخ و زمان فعلی سیستم                |
| `date +"%T"` یا `+"%H:%M:%S"` | نمایش فقط ساعت                               |
| `date +"%F"` یا `+"%Y-%m-%d"` | نمایش فقط تاریخ                              |
| `time COMMAND`                | محاسبه زمان اجرای یک دستور (real، user، sys) |
| `cal`                         | نمایش تقویم ماه جاری                         |
| `cal 1 2025`                  | نمایش تقویم یک ماه خاص (ژانویه ۲۰۲۵)         |
| `cal 2025`                    | نمایش تقویم یک سال کامل                      |
| `cal -y`                      | نمایش تقویم سال جاری                         |
| `cal --monday`                | شروع هفته از دوشنبه                          |
| `cal -h`                      | تقویم با هایلایت کردن تاریخ امروز            |

### 🧾 فرمت‌های پرکاربرد در `date`:

| کد      | معنی                   |
| ------- | ---------------------- |
| `%Y`    | سال ۴ رقمی             |
| `%m`    | ماه عددی (01–12)       |
| `%d`    | روز ماه                |
| `%H`    | ساعت ۲۴ ساعته          |
| `%M`    | دقیقه                  |
| `%S`    | ثانیه                  |
| `%A/%a` | نام کامل/مخفف روز هفته |
| `%B/%b` | نام کامل/مخفف ماه      |
| `%p`    | AM / PM                |

